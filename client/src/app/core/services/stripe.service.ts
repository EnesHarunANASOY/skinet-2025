...
export class StripeService {
  ...
  private paymentElemnets?: StripePaymentElement;
  ...
  async initializeElements()
  {
    if(!this.elements)
    {
      ...
      if(stripe)
      {
        const cart = await firstValueFrom(this.createOrUpdatePaymnetIntent());
  ...
  {
    if(!this.paymentElemnets)
    {
      const elements = await this.initializeElements();
      if(elements)
      {
        this.paymentElemnets = elements.create('payment');
      }
      else{
  ...
  disposeElements()
  {
    ...
    this.paymentElemnets=undefined;
  }
